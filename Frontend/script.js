...
function applyTheme(theme) {
  ...
  try {
    window.dispatchEvent(
      new CustomEvent('zenrix-theme-changed', {
...
async function loadProducts(category = null) {
  try {
    const container =
      document.getElementById('productsGrid') || document.getElementById('featuredProducts');
    if (!container) return;
...
function hydrateHeroSpotlight(products = []) {
  ...
  if (!container) return;
  const featuredList = Array.isArray(products)
...
function hydrateSpotlightBundle(products = []) {
  ...
  if (hero) {
    ...
    if (Number.isFinite(Number(hero.spotlightPrice)) && priceEl)
  ...
  const configuredItems = Array.isArray(hero?.spotlightItems)
    ? hero.spotlightItems.filter(Boolean)
    : [];
  ...
  const categoryLabels = [
    ...new Set(bundleItems.map((item) => (item.category || 'Essentials').toLowerCase())),
  ].map((label) => label.charAt(0).toUpperCase() + label.slice(1));
  const categorySummary = categoryLabels.slice(0, 2).join(' • ');

  listEl.innerHTML = bundleItems
    .map((item) => {
      const image = item.image || 'https://via.placeholder.com/160?text=Zenrix';
      const category = (item.category || 'Essentials').replace(/\b\w/g, (char) =>
        char.toUpperCase()
      );
      return `
  ...
  if (metaEl)
    metaEl.textContent = `Includes ${bundleItems.length} picks${categorySummary ? ` • ${categorySummary}` : ''}`;
  if (titleEl)
    titleEl.textContent = bundleItems.length >= 3 ? 'Spotlight Weekend Bundle' : 'Spotlight Picks';
  if (copyEl)
    copyEl.textContent = `Hand matched ${categorySummary || 'everyday'} essentials built to layer together.`;
  if (ctaEl) {
  ...
  container.innerHTML = products
    .map((product) => {
      ...
      const imageSrc =
        ...
        'https://via.placeholder.com/400x400?text=Zenrix';
...
function getDemoProduct() {
  return {
    ...
    price: 129.99,
    description:
      'Signature adaptive sound, obsidian acoustic mesh, and sculpted memory-foam cushions built for marathon listening sessions.',
    stock: 12,
    rating: 4.9,
    reviewCount: 64,
    image:
      'https://images.unsplash.com/photo-1518443757220-aee51611a37e?auto=format&fit=crop&w=1200&q=80',
    images: [
...
function renderProductGallery(product) {
  ...
  thumbContainer.innerHTML = sources
    .map(
      (src, index) => `
...
async function loadPageContent() {
  try {
    ...
    if (slug === 'index') {
      ...
      container.innerHTML =
        '<p class="text-gray-600">Content not found. Please add this page from the Admin Dashboard.</p>';
    }
  } catch (err) {
    const container = document.getElementById('pageContent');
    if (!container) return;
...
function getStarRating(rating) {
  const full =
    '<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-yellow-400 inline-block mr-0.5" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.955a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.455a1 1 0 00-.364 1.118l1.287 3.955c.3.921-.755 1.688-1.54 1.118L10 13.347l-3.448 2.441c-.784.57-1.838-.197-1.539-1.118l1.287-3.955a1 1 0 00-.364-1.118L2.568 9.382c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69L9.049 2.927z"/></svg>';
  const half =
    '<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-yellow-400 inline-block mr-0.5" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.955a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.455a1 1 0 00-.364 1.118l1.287 3.955c.3.921-.755 1.688-1.54 1.118L10 13.347V2.927z"/></svg>';
  const empty =
    '<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-yellow-300 inline-block mr-0.5" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.955a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.455a1 1 0 00-.364 1.118l1.287 3.955c.3.921-.755 1.688-1.54 1.118L10 13.347l-3.448 2.441c-.784.57-1.838-.197-1.539-1.118l1.287-3.955a1 1 0 00-.364-1.118L2.568 9.382c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69L9.049 2.927z"/></svg>';
  let stars = '';
  for (let i = 1; i <= 5; i++) {
...
function renderCartPage(items) {
  ...
  container.innerHTML = items
    .map((item) => {
      ...
      const decClasses =
        qty <= 1
          ? 'px-3 py-2 text-slate-500 cursor-not-allowed bg-white/5'
          : 'px-3 py-2 text-white hover:bg-white/10';
      return `
...
function renderPaymentMethods(settings) {
  ...
  if (!methods.length) {
    methodsWrapper.innerHTML =
      '<p class="text-sm text-red-500">No payment methods enabled. Please contact support.</p>';
    return;
  }

  methodsWrapper.innerHTML = methods
    .map(
      (method, index) => `
...
function handlePaymentMethodChange(method) {
  ...
  if (method === 'cod') {
    content =
      '<p class="text-sm text-gray-600">Pay in cash when the courier delivers your parcel. Please have the exact amount ready.</p>';
...
async function submitCheckoutOrder(event) {
  ...
  if (method !== 'cod' && !instructionsAckValue) {
    statusEl &&
      (statusEl.textContent = 'Please confirm you have followed the payment instructions.');
    return;
  }
